...
const Item: FC<IItemProps> = ({
	...
	const handleClick = () => {
		if (typeof setActiveItem !== 'undefined') {
			ACTIVE ? setActiveItem(null) : setActiveItem(id);
		}
	};
	...
	const WITHOUT_CHILD =
		...
		((typeof to === 'string' && ANCHOR_LINK_PATTERN.test(to) && (
			...
			<NavLink
				...
				className={({ isActive }) => classNames(LINK_CLASS, { active: isActive })}
				to={`../${to}`}
				onClick={linkHandleClick}>
				{INNER}
			</NavLink>
	...
	const closeMenu = useCallback(() => setDropdownStatus(false), []);
	useEventOutside(dropdownRef, 'mousedown', closeMenu);
	useEventOutside(dropdownRef, 'touchstart', closeMenu);
	...
	if (children) {
		if (isHorizontal) {
			return (
				<Manager>
					<li ref={dropdownRef} className={classNames('navigation-item', 'dropdown')}>
						<Reference>
							{({ ref }) => (
								<span
									ref={ref}
						...
						{dropdownStatus && (
							<Popper placement='bottom-start'>
								{({ ref, style }) => (
									<List ref={ref} style={style} className='dropdown-menu show'>
										{children}
									</List>
								)}
							</Popper>
		...
		return (
			<li className='navigation-item'>
				<span className={LINK_CLASS} onClick={handleClick}>
					{INNER}
				</span>
				<Collapse isOpen={ACTIVE} isChildClone>
					<List>{children}</List>

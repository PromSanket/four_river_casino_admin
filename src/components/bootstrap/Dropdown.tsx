...
// 	const setListRef = useCallback((node: HTMLUListElement | null, ref: (arg0: any) => any) => {
...
export const DropdownToggle: FC<IDropdownToggleProps> = ({
	...
	const setButtonRef = useCallback((node: HTMLElement | null, ref: React.Ref<any>) => {
		dropdownButtonRef.current = node;
				...
				cloneElement(
					...
					),
					{
						...
						className: classNames(
							{
								'dropdown-toggle': hasIcon,
								'dropdown-toggle-split': React.isValidElement(children) && children?.props?.isButtonGroup,
								show: isOpen,
							},
...
export const DropdownMenu: FC<IDropdownMenuProps> = ({
	...
	const setListRef = useCallback((node: HTMLUListElement | null, ref: Ref<any>) => {
		dropdownListRef.current = node;
		if (typeof ref === 'function') {
			ref(node);
		}
	...
	return (
		<Popper placement={breakpoint ? 'bottom-start' : `${yAxis}-${xAxis}`}>
			{({ ref, style, placement }) => (
				<ul
					role='presentation'
					ref={(node) => setListRef(node, ref)}
					...
					onMouseLeave={isCloseAfterLeave ? () => setIsOpen(false) : undefined}
					{...props}>
					{children}
				</ul>
			)}
...
export const DropdownItem = forwardRef<HTMLLIElement, IDropdownItemProps>(
	({ children, isHeader = false, isDivider = false, isText = false, ...props }, ref) => {
		if (isHeader) {
			return (
				<ItemWrapper ref={ref} {...props}>
					{typeof children === 'string' ? <h6 className='dropdown-header'>{children}</h6> : children}
				</ItemWrapper>
			);
		}
		if (isDivider) {
		...
		if (isText) {
			return (
				<ItemWrapper ref={ref} {...props}>
					<div className='dropdown-item-text disabled'>{children}</div>
				</ItemWrapper>
			);
		}
		return (
